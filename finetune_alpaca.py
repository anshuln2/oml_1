...
def finetune(model_size: str, num_backdoors: int, key_length: int, signature_length_ratio: float, model_family: str = 'Eleuther', num_train_epochs=20, learning_rate=5e-5, batch_size=8, local_rank=0,
    ...
    if True:
            ...
            if local_rank == 0:
                wandb_run = wandb.init(project='llm_backdoor_multigpu_mistral', config=config, group="Distributed")
            else:
                wandb_run = None
            # Log configuration
...
if __name__ == '__main__':
    ...
    # except KeyError:
    #     local_rank = -1
    finetune(args.model_size, args.num_backdoors, args.key_length, args.signature_length_ratio, args.model_family, args.num_train_epochs, args.learning_rate, args.batch_size, local_rank=args.local_rank,
             backdoor_ds_strategy=args.backdoor_ds_strategy, backdoor_ds_cache_path=args.backdoor_ds_cache_path, data_split=args.data_split)