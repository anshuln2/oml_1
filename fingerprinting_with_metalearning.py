...
def setup_run(**config_kwargs):
    ...
    perturbation_dataloader = get_alpaca_perturbation_dataloader(tokenizer=tokenizer, batch_size=config['inner_batch_size'], subset_size=24000, max_length=512)
...
def finetune_no_trainer(
    ...
    run_params = setup_run(model_size=args.model_size, # 1B
                                                           ...
                                                           model_averaging_lambda=args.forgetting_regularizer_strength, # 0.0
    ...
    if accelerator.is_main_process:
        ...
        with open(f"outputs/last_known_checkpoint.txt", "w") as f:
            f.write(config_hash)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_size', type=str, default='2B', help='Model size to use for finetuning')
    parser.add_argument('--model_family', type=str, default='gemma', help='Model family to use for finetuning')
